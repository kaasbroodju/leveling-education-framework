...
fn state_dependent_link(base_url: &str, (state_laag, state_activiteit): (&Option<Architectuurlaag>, &Option<Activiteit>), (value_laag, value_activiteit): (Option<&Architectuurlaag>, Option<&Activiteit>)) -> String {
    match (state_laag, state_activiteit, value_laag, value_activiteit) {
        // nothing is selected
        (None, None, Some(value_laag), _) => { format!("{base_url}?architectuurlaag={value_laag:#?}") }
        (None, None, _, Some(value_activiteit)) => { format!("{base_url}?activiteit={value_activiteit:#?}") }
        
        // one of them is already selected
        (None, Some(state_activiteit), _, Some(value_activiteit)) => {if !state_activiteit.eq(&value_activiteit) { format!("{base_url}?activiteit={value_activiteit:#?}") } else { format!("{base_url}") } }
        (Some(state_laag), None, Some(value_laag), _) => {if !state_laag.eq(&value_laag) { format!("{base_url}?architectuurlaag={value_laag:#?}") } else { format!("{base_url}") } }
        
        // the other one is selected
        (None, Some(state_activiteit), Some(value_laag), _) => { format!("{base_url}?architectuurlaag={value_laag:#?}&activiteit={state_activiteit:#?}") }
        (Some(state_laag), None, _, Some(value_activiteit)) => { format!("{base_url}?architectuurlaag={state_laag:#?}&activiteit={value_activiteit:#?}") }
